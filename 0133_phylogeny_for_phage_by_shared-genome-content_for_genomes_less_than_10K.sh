#!/bin/bash
## changes 
## mash sketch -s 100 instead of 1000
## also division by 100 instread of 1000 at awk '{print $5}' results/33_minhash_prophage_SGC/distance/sublist/my_fasta_prophage_copied_all/$i.distance.txt | awk -F'/' '{print $1*100/100 }' | tr ' ' '\n' > results/33_minhash_prophage_SGC/SGC_ANI/tmp/$i.SGC.txt
###############################################################################
#33 minhash prophage
###############################################################################
    ...
    if [ ! -f results/33_minhash_prophage_SGC/tmp/all.distance.for_R.tab ]; then
        ...
        if [ ! -d results/33_minhash_prophage_SGC/msh ] ; then
            ...
            for i1 in $(cat $list_copied_fasta); do
                if [ ! -f results/33_minhash_prophage_SGC/msh/$i1.mash_sketch.msh ] ; then
                    $mash_tool_path/mash sketch -s 100 -p 50 \
        ...
        if [ ! -d results/33_minhash_prophage_SGC/SGC_ANI ] ; then
            ...
            for i in $(cat $list_copied_fasta ); do
                ...
                awk '{print $5}' results/33_minhash_prophage_SGC/distance/sublist/my_fasta_prophage_copied_all/$i.distance.txt | awk -F'/' '{print $1*100/100 }' | tr ' ' '\n' > results/33_minhash_prophage_SGC/SGC_ANI/tmp/$i.SGC.txt
