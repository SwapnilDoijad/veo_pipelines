...
#           b) Remove tandem repeats from the resulting rearrangement
...
def usage():
    ...
    -v                      Verbosity switch to visualize primers across the length of the reads
    ''')
...
def process_etandem_record(full_sequence, verbosity, tandem_min_repeat, tandem_max_repeat, tandem_threshold, tandem_mismatch, tandem_identity_threshold):
    ...
    if len(etandem_output) > 1:
        ...
        allowed_ident_threshold = (
                tandem_identity_threshold - math.floor(t_match_length / float(
            (float(tandem_max_repeat) - float(tandem_min_repeat)) / 80.0)))

        if verbosity:
            if t_match_ident >= allowed_ident_threshold:
                print("Step 2: Removing tandem repeats (identity >= " + str(
                    allowed_ident_threshold) + ") in correctly arranged sequence" + "[length=" + str(
                    t_match_length) + ";score=" + str(t_match_score) + ";ident=" + str(
                    t_match_ident) + ";count=" + str(t_match_count) + ";consensus=" + t_match_consensus + "]")
                print(
                    t_left_string + '\x1b[6;30;44m' + t_tandom_repeat_1 + '\x1b[0m' + '\x1b[6;30;45m' + t_tandom_repeat_2 + '\x1b[0m' + t_right_string)
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "hi:f:r:vl:m:", ["input_file", "forward_primer", "reverse_primer", "minimum_length", "maximum_length"])
    ...
    for seq_record in SeqIO.parse(input_file, "fasta"):
        ...
        # Updated sequence alignment using PairwiseAligner
        aligner = PairwiseAligner()
        aligner.match_score = primer_match_score
        aligner.mismatch_score = primer_mismatch_score
        aligner.open_gap_score = primer_open_gap_score
        aligner.extend_gap_score = primer_extend_gap_score

        forward_orientation_forward_primer_alignment = aligner.align(str(seq_record.seq), forward_primer)
        forward_orientation_reverse_primer_alignment = aligner.align(str(seq_record.seq),
                                                                    str(Seq(reverse_primer).reverse_complement()))
        reverse_orientation_forward_primer_alignment = aligner.align(str(seq_record.seq), reverse_primer)
        reverse_orientation_reverse_primer_alignment = aligner.align(str(seq_record.seq),
                                                                    str(Seq(forward_primer).reverse_complement()))

        # Check if alignments exist before accessing their details
        if forward_orientation_forward_primer_alignment and forward_orientation_reverse_primer_alignment and \
            reverse_orientation_forward_primer_alignment and reverse_orientation_reverse_primer_alignment:
                if numpy.mean([forward_orientation_forward_primer_alignment[0][2],
                        forward_orientation_reverse_primer_alignment[0][2]]) > numpy.mean(
                [reverse_orientation_forward_primer_alignment[0][2], reverse_orientation_reverse_primer_alignment[0][2]]):
        ...
        if numpy.mean([forward_orientation_forward_primer_alignment[0][2],
            forward_orientation_reverse_primer_alignment[0][2]]) > numpy.mean(
                ...
                f_match_string = str(
                    forward_orientation_forward_primer_alignment[0][0][f_match_left_position:f_match_right_position])
            ...
            r_match_string = str(
                forward_orientation_reverse_primer_alignment[0][0][r_match_left_position:r_match_right_position])
            ...
            full_sequence = process_etandem_record(full_sequence, verbosity, tandem_min_repeat, tandem_max_repeat,
                                                tandem_threshold, tandem_mismatch, tandem_identity_threshold)

            if not verbosity:
                if minimum_read_length_threshold <= len(full_sequence) <= maximum_read_length_threshold:
            ...
            f_match_string = str(
                reverse_orientation_forward_primer_alignment[0][0][f_match_left_position:f_match_right_position])
            ...
            r_match_string = str(
                reverse_orientation_reverse_primer_alignment[0][0][r_match_left_position:r_match_right_position])
            ...
            # Handle the case where no alignments were found for the current sequence
            print("No alignments found for sequence:", seq_record.id)
