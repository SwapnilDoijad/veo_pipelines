...
###############################################################################
    ...
    echo "#####################################################################"
    head -1 /veodata/03/databases/mgnify/mgnify.sample.index.*.tab | tr "\t" "\n" 
    #head -1 /veodata/03/databases/mgnify/mgnify.sample.index.*.tab | tr "\t" "\n"
    echo "#####################################################################"
    ...
    echo "searching..."
###############################################################################
    parameter_number=$(awk '$1 == "'$parameter'" {print $2}' /veodata/03/databases/mgnify/supporting_file.tab)
###############################################################################

    count_for_user_input_1=$(awk -F'\t' 'NR==1 { for (i=1;i<=NF;++i) if ($i=="'$parameter'") { n=i; break }} { print $1, $n }' /veodata/03/databases/mgnify/mgnify.sample.index.*.tab | grep -i $user_input_1 | wc -l)
    
    ## studies_for_query
    awk -F'\t' 'NR==1 { for (i=1;i<=NF;++i) if ($i=="'$parameter'") { n=i; break }} { print $1, $n }' /veodata/03/databases/mgnify/mgnify.sample.index.*.tab | grep -i $user_input_1 | awk '{print $1}' | sed -e 's/_/\n/g' | sort -u > $mydir/mgnify_study_ids.txt
    ...
    for F1 in $(cat $mydir/mgnify_study_ids.txt); do
        grep $F1 /veodata/03/databases/mgnify/mgnify.study.index.*.tab >> $mydir/mgnify_study_details.txt
    ...
    awk 'BEGIN{IGNORECASE=1} $8 ~ "spain" {print $0}' /veodata/03/databases/mgnify/mgnify.sample.index.*.tab >> sample_metadata_"$parameter"_"$user_input_1".tab
